...
def filter_h_timeaware(quadruples_to_filter, target_h, target_r, target_t,target_time, num_entities):
    target_h, target_r, target_t, target_time = int(target_h), int(target_r), int(target_t), int(target_time)
    filtered_h = []

    # Do not filter out the test quadruple, since we want to predict on it
    if (target_h, target_r, target_t, target_time) in quadruples_to_filter:
        quadruples_to_filter.remove((target_h, target_r, target_t,target_time))
    # Do not consider an object if it is part of a quadruple to filter
    for h in range(num_entities):
        if (h, target_r, target_t,target_time) not in quadruples_to_filter:
            filtered_h.append(h)
    return torch.LongTensor(filtered_h)

def filter_t_timeaware(quadruples_to_filter, target_h, target_r, target_t, target_time, num_entities):
    target_h, target_r, target_t = int(target_h), int(target_r), int(target_t)
    filtered_t = []

    # Do not filter out the test quadruple, since we want to predict on it
    if (target_h, target_r, target_t, target_time) in quadruples_to_filter:
        quadruples_to_filter.remove((target_h, target_r, target_t))
    # Do not consider an object if it is part of a quadruple to filter
    for t in range(num_entities):
        if (target_h, target_r, t, target_time) not in quadruples_to_filter:
            filtered_t.append(t)
    return torch.LongTensor(filtered_t)

def get_filtered_rank_timeaware(num_entity, score, h, r, t, time, test_size, quadruples_to_filter, entity):
    """ Perturb object in the quadruples
    """
    num_entities = num_entity
    ranks = []
    ...
    for idx in range(test_size):
        ...
        target_r = r[idx]
        target_t = t[idx]
        target_time =time[idx]
        # print('t',target_t)
        if entity == 'object':
            filtered_t = filter_t_timeaware(quadruples_to_filter, target_h, target_r, target_t, target_time, num_entities)
            target_t_idx = int((filtered_t == target_t).nonzero())
            _, indices = torch.sort(score[idx][filtered_t], descending=True)
            rank = int((indices == target_t_idx).nonzero())
        if entity == 'subject':
            filtered_h = filter_h_timeaware(quadruples_to_filter, target_h, target_r, target_t, target_time,num_entities)
            target_h_idx = int((filtered_h == target_h).nonzero())
            _, indices = torch.sort(score[idx][filtered_h], descending=True)
            rank = int((indices == target_h_idx).nonzero())
...
def calc_filtered_mrr_timeaware(num_entity, score, train_quadruples, valid_quadruples, test_quadruples, entity, hits=[]):
    with torch.no_grad():
        h = test_quadruples[:, 0]
        r = test_quadruples[:, 1]
        t = test_quadruples[:, 2]
        time = test_quadruples[:,3]
        test_size = test_quadruples.shape[0]

        train_quadruples = torch.Tensor([[quad[0], quad[1], quad[2],quad[3]] for quad in train_quadruples])
        valid_quadruples = torch.Tensor([[quad[0], quad[1], quad[2],quad[3]] for quad in valid_quadruples])
        test_quadruples = torch.Tensor([[quad[0], quad[1], quad[2],quad[3]] for quad in test_quadruples])

        quadruples_to_filter = torch.cat([train_quadruples, valid_quadruples, test_quadruples]).tolist()

        quadruples_to_filter = {tuple(quadruples) for quadruples in quadruples_to_filter}

        ranks = get_filtered_rank_timeaware(num_entity, score, h, r, t, time, test_size, quadruples_to_filter, entity)
...
def calc_filtered_test_mrr_timeaware(num_entity, score, train_quadruples, valid_quadruples, valid_quadruples2, test_quadruples, entity, hits=[]):
    with torch.no_grad():
        h = test_quadruples[:, 0]
        r = test_quadruples[:, 1]
        t = test_quadruples[:, 2]
        ...
        train_quadruples = torch.Tensor([[quad[0], quad[1], quad[2], quad[3]] for quad in train_quadruples])
        valid_quadruples = torch.Tensor([[quad[0], quad[1], quad[2], quad[3]] for quad in valid_quadruples])
        valid_quadruples2 = torch.Tensor([[quad[0], quad[1], quad[2], quad[3]] for quad in valid_quadruples2])
        test_quadruples = torch.Tensor([[quad[0], quad[1], quad[2], quad[3]] for quad in test_quadruples])
        print(train_quadruples.shape)
        print(valid_quadruples.shape)
        print(valid_quadruples2.shape)
        print(test_quadruples.shape)
        quadruples_to_filter = torch.cat([train_quadruples, valid_quadruples, valid_quadruples2, test_quadruples]).tolist()
        quadruples_to_filter = {tuple(quadruples) for quadruples in quadruples_to_filter}
