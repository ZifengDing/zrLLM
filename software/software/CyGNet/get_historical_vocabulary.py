...
for tim in tqdm(train_times):
    ...
    inverse_train_data[:, 1] = inverse_train_data[:, 1] + num_r # create inverse relation
    train_new_data = torch.cat([train_new_data, inverse_train_data])
    # get object entities
    row = train_new_data[:, 0] * 2 * num_r + train_new_data[:, 1]
    col = train_new_data[:, 2]
    d = np.ones(len(row))
    tail_seq = sp.csr_matrix((d, (row, col)), shape=(num_e * 2 * num_r, num_e))
