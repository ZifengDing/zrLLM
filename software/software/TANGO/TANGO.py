...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epoch', type=int, default=500, help='number of maximum epoch')
    ...
    parser.add_argument('--dataset', type=str, default='ICEWS21_foul', help='dataset name')
    ...
    parser.add_argument('--gamma_init', type=float, default=1, help='initialiation of gamma value')
    parser.add_argument('--gamma_fix', type=int, default=1, help='whether to fix gamma, specify 0 if unfix')
    ...
    ts2emb = {}
    dict_path = modelpth + 'submission/emb_all_extraGNN_submission_acl.pkl'
    print(dict_path)
    # assert 0
    ...
    if val_exist:
        if args.induct_test:
            ...
            ts_max, num_e, num_rel, train_timestamps, test_timestamps, val_timestamps, train_adj, test_adj, val_adj, train_triple, test_triple, val_triple, \
        ...
        ts_max, num_e, num_rel, train_timestamps, test_timestamps, train_adj, test_adj, train_triple, test_triple, train_1nei, test_1nei, t_indep_trp, train_so2r, test_so2r \
    ...
    ###### for relation path ######
    train_path, valid_path, test_path = load_for_path(args.dataset)
    train_list, train_times = split_by_time(train_path)
    valid_list, valid_times = split_by_time(valid_path)
    test_list, test_times = split_by_time(test_path)
    history_valid_time_nogt = valid_times[0]
    ...
    if args.induct_test == False and args.test == False:
        for epoch in range(args.num_epoch):
            ...
            for step, (sub_in, rel_in, obj_in, lab_in, sub_tar, rel_tar, obj_tar, lab_tar, tar_ts, in_ts, edge_idlist, \
                ...
                optim.zero_grad()
                model.train()
                # print(tar_ts[0].item()/args.scale * ts_max)
                cur_ts = int(tar_ts[0].item() / args.scale * ts_max)
                # print(cur_ts)
                if tar_ts[0].item() / args.scale * ts_max - cur_ts > 0.9:
                    cur_ts += 1

                if args.path:
                    all_rel_path = sp.load_npz(
                        '{}/history/rel_path_{}.npz'.format(args.dataset, cur_ts))
                    rel_path_idx = sub_tar[0] * num_e + obj_tar[0]
                    rel_path = torch.Tensor(all_rel_path[rel_path_idx].todense())
                    one_hot_rel_path = rel_path.masked_fill(rel_path != 0, 1)

                    one_hot_rel_path = one_hot_rel_path.to(args.device)
                else:
                    one_hot_rel_path = None

                # forward
                t3 = time.time()
                loss = model(sub_tar, rel_tar, obj_tar, lab_tar, in_ts, tar_ts, edge_idlist, edge_typelist, edge_jump_id, edge_jump_w, rel_jump, one_hot_rel_path, cur_ts)
                t4 = time.time()
                ftime += (t4 - t3)
            ...
            if (epoch+1) % args.test_step == 0:
                if val_exist:
                    results, _ = predict(val_loader, model, args, num_e, num_rel, test_adjmtx, logger, ts2emb, ts_max, history_valid_time_nogt, valid_list)
                else:
                    results, _ = predict(test_loader, model, args, num_e, num_rel, test_adjmtx, logger, ts2emb, ts_max, history_test_time_nogt, test_list)
                print("==========VALID==========")
                print("===========RAW===========")
                ...
                results_, _ = predict(test_loader, model, args, num_e, num_rel, test_adjmtx, logger, ts2emb, ts_max, history_test_time_nogt, test_list)
                print("===========TEST==========")
                print("===========RAW===========")
                print("Epoch {}, HITS10 {}".format(epoch + 1, results_['hits@10_raw']))
                print("Epoch {}, HITS3 {}".format(epoch + 1, results_['hits@3_raw']))
                print("Epoch {}, HITS1 {}".format(epoch + 1, results_['hits@1_raw']))
                print("Epoch {}, MRR {}".format(epoch + 1, results_['mrr_raw']))
                # print("Epoch {}, MAR {}".format(epoch + 1, results_['mar_raw']))

                print("=====TIME AWARE FILTER=====")
                print("Epoch {}, HITS10 {}".format(epoch + 1, results_['hits@10']))
                print("Epoch {}, HITS3 {}".format(epoch + 1, results_['hits@3']))
                print("Epoch {}, HITS1 {}".format(epoch + 1, results_['hits@1']))
                print("Epoch {}, MRR {}".format(epoch + 1, results_['mrr']))
                # print("Epoch {}, MAR {}".format(epoch + 1, results_['mar']))

                print("====TIME UNAWARE FILTER====")
                print("Epoch {}, HITS10 {}".format(epoch + 1, results_['hits@10_ind']))
                print("Epoch {}, HITS3 {}".format(epoch + 1, results_['hits@3_ind']))
                print("Epoch {}, HITS1 {}".format(epoch + 1, results_['hits@1_ind']))
                print("Epoch {}, MRR {}".format(epoch + 1, results_['mrr_ind']))
                # print("Epoch {}, MAR {}".format(epoch + 1, results_['mar_ind']))

                logger.info("===========TEST==========")
                logger.info("===========RAW===========")
                logger.info("Epoch {}, HITS10 {}".format(epoch + 1, results_['hits@10_raw']))
                logger.info("Epoch {}, HITS3 {}".format(epoch + 1, results_['hits@3_raw']))
                logger.info("Epoch {}, HITS1 {}".format(epoch + 1, results_['hits@1_raw']))
                logger.info("Epoch {}, MRR {}".format(epoch + 1, results_['mrr_raw']))
                # logger.info("Epoch {}, MAR {}".format(epoch + 1, results['mar_raw']))

                logger.info("=====TIME AWARE FILTER=====")
                logger.info("Epoch {}, HITS10 {}".format(epoch + 1, results_['hits@10']))
                logger.info("Epoch {}, HITS3 {}".format(epoch + 1, results_['hits@3']))
                logger.info("Epoch {}, HITS1 {}".format(epoch + 1, results_['hits@1']))
                logger.info("Epoch {}, MRR {}".format(epoch + 1, results_['mrr']))
                # logger.info("Epoch {}, MAR {}".format(epoch + 1, results['mar']))

                logger.info("====TIME UNAWARE FILTER====")
                logger.info("Epoch {}, HITS10 {}".format(epoch + 1, results_['hits@10_ind']))
                logger.info("Epoch {}, HITS3 {}".format(epoch + 1, results_['hits@3_ind']))
                logger.info("Epoch {}, HITS1 {}".format(epoch + 1, results_['hits@1_ind']))
                logger.info("Epoch {}, MRR {}".format(epoch + 1, results_['mrr_ind']))
                # logger.info("Epoch {}, MAR {}".format(epoch + 1, results['mar_ind']))
        ...
        results, ts2emb_result = predict(test_loader, model, args, num_e, test_adjmtx, logger, ts2emb)
        print("===========RAW===========")
        print("Epoch {}, HITS10 {}".format(epoch + 1, results['hits@10_raw']))
        print("Epoch {}, HITS3 {}".format(epoch + 1, results['hits@3_raw']))
        print("Epoch {}, HITS1 {}".format(epoch + 1, results['hits@1_raw']))
