...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, time_list, history_time_nogt, log_name, mode, rel2obj=None, test_list2=None, all_ans_list2=None, all_ans_r_list2=None):
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        if use_cuda:
            ...
            # one_hot_rel_path_ent = one_hot_rel_path_ent.cuda()
        if args.multi_step:
            test_triples, final_score, final_r_score = model.predict(history_glist, num_rels, static_graph, test_triples_input, one_hot_tail_seq, one_hot_rel_seq, one_hot_rel_path, use_cuda, history_time_nogt)
        else:
            test_triples, final_score, final_r_score = model.predict(history_glist, num_rels, static_graph,
                                                                     test_triples_input, one_hot_tail_seq,
                                                                     one_hot_rel_seq, one_hot_rel_path, use_cuda,
                                                                     history_time_nogt)
        mrr_filter_snap, mrr_snap, rank_raw, rank_filter = utils.get_total_rank(test_triples, final_score, all_ans_list[time_idx], eval_bz=10, rel_predict=0)
        ...
        if test_list2 != None:
            ...
            all_rel_path = sp.load_npz(
                '../data/{}/history/rel_path_{}.npz'.format(args.dataset, history_time_nogt))
            rel_path_idx = histroy_data[:, 0] * num_nodes + histroy_data[:, 2]
            tail_seq_idx = histroy_data[:, 0] * num_rels + histroy_data[:, 1]

            tail_seq = torch.Tensor(all_tail_seq[tail_seq_idx].todense())
            one_hot_tail_seq = tail_seq.masked_fill(tail_seq != 0, 1)

            rel_seq = torch.Tensor(all_rel_seq[rel_path_idx].todense())
            one_hot_rel_seq = rel_seq.masked_fill(rel_seq != 0, 1)
            ...
            test_triples2, final_score2, final_r_score2 = model.predict(history_glist, num_rels, static_graph,
                                                                        test_triples_input2, one_hot_tail_seq, one_hot_rel_seq, one_hot_rel_path, use_cuda,
                                                                        history_time_nogt)
        ...
        if args.multi_step:
            if not args.relation_evaluation:
                ...
                if test_list2 != None:
                    predicted_snap2 = utils.construct_snap(test_triples2, num_nodes, num_rels, final_score2, args.topk)
            else:
    ...
    mrr_raw, hit_result_raw = utils.stat_ranks(ranks_raw, "raw_ent", log_name)
    mrr_filter, hit_result_filter = utils.stat_ranks(ranks_filter, "filter_ent", log_name)
    ...
    mrr_raw_r, hit_result_raw_r = 0, 0
    mrr_filter_r, hit_result_filter_r = 0, 0

    mrr_raw2, hit_result_raw2 = utils.stat_ranks(ranks_raw2, "test raw_ent", log_name)
    mrr_filter2, hit_result_filter2 = utils.stat_ranks(ranks_filter2, "test filter_ent", log_name)
...
def run_experiment(args, history_len=None, n_layers=None, dropout=None, n_bases=None, angle=None, history_rate=None):
    # zero_log = '../results/{}.{}.gs'.format(args.dataset, args.encoder + "-" + args.decoder + "-" + args.save)
    o_f = open('zero_log.txt', 'w')
    name = 'results/' + args.dataset + '_' + time.strftime('%Y_%m_%d') + '_' + time.strftime('%H:%M:%S') + '_' + 'LLM-' + str(args.LLM) + '_' + 'path-' + str(args.path) + '_' + 'method-' + str(args.path_method) + '_' + 'concept-' + str(args.concept) + '_' + 'gamma-' + str(args.gamma_init)
    ...
    rel2obj = torch.zeros((2 * data.num_rels, data.num_nodes))
    # print(rel2obj.shape)
    for quad in data.train:
        # print(len(data.train))
        # print(quad)
        # assert 0
        rel2obj[quad[1]][quad[2]] += 1
        rel2obj[quad[1] + data.num_rels][quad[0]] += 1

    num_nodes = data.num_nodes
    num_rels = data.num_rels
    if args.dataset == "ICEWS14s":
        num_times = len(train_list) + len(valid_list) + len(test_list) + 1
    elif args.dataset == 'ACLED':
        num_times = len(train_list) + len(test_list)
    elif args.dataset == 'ICEWS21':
        num_times = len(train_list) + len(test_list)
    elif args.dataset == 'ICEWS22':
        num_times = len(train_list) + len(test_list)
    else:
    ...
    if args.test and os.path.exists(model_state_file):
        mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r, hit_result_raw, hit_result_filter, hit_result_raw_r, hit_result_filter_r = test(model,
                                                            ...
                                                            all_ans_list2=all_ans_list_test,
                                                            all_ans_r_list2=all_ans_list_r_test)
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                # rel path
                all_rel_path = sp.load_npz(
                    '../data/{}/history/rel_path_{}.npz'.format(args.dataset, train_times[train_sample_num]))
                # print(train_times[train_sample_num], len(histroy_data[:, 0]))
                # print(all_rel_path.shape)
                rel_path_idx = histroy_data[:, 0] * num_nodes + histroy_data[:, 2]
                rel_path = torch.Tensor(all_rel_path[rel_path_idx].todense())
                one_hot_rel_path = rel_path.masked_fill(rel_path != 0, 1)
                ...
                loss_e, loss_r, loss_static = model.get_loss(history_glist, output[0], static_graph, one_hot_tail_seq, one_hot_rel_seq, one_hot_rel_path, use_cuda, train_times[train_sample_num])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--path_len", type=int, default=-1,
                        help="length of historical path")
    ...
    if args.grid_search:
        ...
        if args.dataset == "ACLED":
            hp_range_ = hp_range
        if args.dataset == "ICEWS21":
            hp_range_ = hp_range
