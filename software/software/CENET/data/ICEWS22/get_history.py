...
args.add_argument('--dataset', type=str, default='ICEWS22')
...
rel_dim1 = []
rel_dim2 = []
for i, tim in enumerate(tqdm(all_times)):
    print(ts[i])
    train_new_data = np.array([[quad[0], quad[1], quad[2], quad[3]] for quad in all_data if (quad[3] < tim)])
    if tim != all_times[0]:
        rel_dim1.extend(ts[i] * train_new_data[:, 0] * num_e + train_new_data[:, 2])
        rel_dim2.extend(train_new_data[:, 3] * num_r + train_new_data[:, 1])
rel_path_d = np.ones(len(rel_dim1))
rel_path = sp.csr_matrix((rel_path_d, (rel_dim1, rel_dim2)), shape=(num_ts * num_e * num_e, num_ts * num_r))
        # for
sp.save_npz('history/rel_path_all.npz', rel_path)
