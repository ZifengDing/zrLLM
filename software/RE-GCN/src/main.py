...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, time_list, history_time_nogt, mode, log_name, test_list2=None, all_ans_list2=None, all_ans_r_list2=None):
    """
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        test_triples, final_score, final_r_score = model.predict(history_glist, num_rels, static_graph, test_triples_input, one_hot_rel_path, use_cuda, history_time_nogt)

        mrr_filter_snap_r, mrr_snap_r, rank_raw_r, rank_filter_r = utils.get_total_rank(test_triples, final_r_score, all_ans_r_list[time_idx], eval_bz=10, rel_predict=1)
        ...
        if test_list2 != None:
            ...
            mrr_filter_snap_r, mrr_snap_r, rank_raw_r, rank_filter_r = utils.get_total_rank(test_triples2, final_r_score2,
                                                                                            all_ans_r_list2[time_idx],
                                                                                            eval_bz=10, rel_predict=1)
        ...
        #
        # ####
        # predicted_new_snap = torch.from_numpy(np.concatenate([predicted_snap, predicted_snap2], axis=0)).to("cuda")
        # ####
        idx += 1
    
    mrr_raw = utils.stat_ranks(ranks_raw, "raw_ent", log_name)
    mrr_filter = utils.stat_ranks(ranks_filter, "filter_ent", log_name)
    mrr_raw_r = utils.stat_ranks(ranks_raw_r, "raw_rel", log_name)
    mrr_filter_r = utils.stat_ranks(ranks_filter_r, "filter_rel", log_name)

    mrr_raw2 = utils.stat_ranks(ranks_raw2, "test raw_ent", log_name)
    mrr_filter2 = utils.stat_ranks(ranks_filter2, "test filter_ent", log_name)
    ...
    if args.test and os.path.exists(model_state_file):
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                loss_e, loss_r, loss_static = model.get_loss(history_glist, output[0], static_graph, one_hot_rel_path, use_cuda, train_times[train_sample_num])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gamma_init', type=float, default=0.01,
                        help='initialiation of gamma value')
